...
const { label = '', value, symbol= '', decimals = 18, loading = false, disabled = false } = defineProps({ 
    label: String, 
    value: BigInt, 
    symbol: String, 
    decimals: Number, 
        ...
        <div class="flex items-stretch flex-auto">
            <span
                class="w-14 flex items-center justify-center border-y border-s border-surface-300 dark:border-surface-700 rounded-s-md overflow-hidden">{{
                    symbol }}</span>
            <InputText 
                :value="formatted" readonly pt:root="flex-1 rounded-s-none rounded-e-none"
            ...
            <span
                ...
                <SecondaryButton 
                    icon="pi pi-refresh" variant="text" :disabled="disabled" pt:root="rounded-none"
                    :loading="loading" @click=" emit('update')" />
