  ...
  <Card class="card-2" pt:body:class="px-0 pb-0 md:px-5 md:pb-5">
    ...
    <template #content>
      <div class="flex flex-col gap-2 text-sm">
        ...
        <InputAmount
          v-model="amount" symbol="RUBR" label="Amount" :decimals="8" :disabled="!contract || running || disabled"
          :invalid="invalidAmount" />

        <div class="flex justify-normal pt-2 gap-x-4 gap-y-2 text-sm">
          <DangerButton class="min-w-32" icon="pi pi-play" label="Execute" :loading="running" :disabled="!contract || running || disabled" @click="execute" />
