...
const execute = async () => {
  if (unref(contract) && validate()) {
    try {
      ...
      txResult.value = await transfer(unref(address), biAmount);
    } finally {
      running.value = false;
    }
  }
...
<template>
  <Card class="card-2" pt:body:class="px-0 pb-0 md:px-5 md:pb-5">
    <template #title>
      <div class="text-sm">Transfer</div>
    ...
    <template #content>
      <div class="flex flex-col gap-2 text-sm">
        ...
        <InputAddress
v-model="address" :disabled="!contract || running || disabled" label="To (address)"
