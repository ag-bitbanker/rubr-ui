...
const { label, disabled, invalid=false } = defineProps(['label','disabled', 'invalid'])
...
<template>
    <div class="flex flex-col">
        <div class="flex flex-row items-center justify-between">
            ...
            <SecondaryButton v-if="account?.address" variant="text" label="My wallet" size="small" @click="model = account.address"/>
            <SecondaryButton v-if="addresses?.treasury" variant="text" label="Treasury" size="small" @click="model = addresses.treasury"/>
            </div>
        </div>
        <div class="flex items-stretch flex-auto">
            <InputText pt:root="flex-1 rounded-s-md rounded-e-none" v-model="model"
                aria-describedby="account" :disabled="disabled"/>
            <span
