...
const { label, disabled = false, invalid = false } = defineProps(['label', 'disabled', 'invalid'])
        ...
        <div class="flex flex-row items-center justify-between">
            ...
            <div class="flex flex-row items-center">
                <Button v-if="account?.address" variant="text" pt:root:class="p-small:p-small:py-[0.175rem]"
                    label="My wallet" size="small" @click="model = account.address" :disabled="disabled" />
                <ContrastButton v-if="addresses?.treasury" variant="text" pt:root:class="p-small:p-small:py-[0.175rem]"
                    label="Treasury" size="small" @click="model = addresses.treasury" :disabled="disabled" />
            </div>
        </div>
        <div class="flex items-stretch flex-auto">
            <InputText pt:root="flex-1 rounded-s-md rounded-e-none" v-model="model" aria-describedby="account"
                :disabled="disabled" />
            <span
